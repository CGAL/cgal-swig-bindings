...
if [ $(python -c "import sys; print(sys.version_info.major)") = "3" ] && [ "$1" = "Linux" ]; then
  ...
  python setup.py bdist_wheel --cgal-dir=/usr/local/opt/cgal/lib/cmake/CGAL --keep-temp --python-root=/Library/Frameworks/Python.framework/Versions/$MB_PYTHON_VERSION
